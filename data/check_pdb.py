...
def samplify_pdb(file_path):
    ...
    for k, v in compound.items():
        print(k)
        mol_str = v['molecule']
        if 'ligand' in mol_str:
            ligand.append(v['chain'][0])

        if 'receptor' in mol_str:
            receptor.append(v['chain'][0])

        if 'light chain' in mol_str:

    idx = 1
    while len(ligand)<1 and len(receptor)<1:
        v = compound[idx]
        mol_str = v['molecule']
...
#                 except KeyError:
#                     print('> KeyError in ', '>chain:' + chain_id, residue.resname, residue.get_id())
#                     raise KeyError
